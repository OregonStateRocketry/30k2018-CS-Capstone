...
class Simulator(object):
    def __init__(self, rate, random, state):
        ...
        self.random = random
        if state == 0:
            self.state = rocketState.PreLaunchPhase()
        if state == 1:
            self.state = rocketState.PrimaryEnginePhase()
        if state == 2:
            self.state = rocketState.SecondaryEnginePhase()
        if state == 3:
            self.state = rocketState.DescentPhase()
