...
class MPU6050(object):
    ...
    def __init__(self, pi, gpio, address=0x69,
        ...
        self.bus.write_byte_data(
            self.address, MPU6050.GYRO_CONFIG, self.gyro_scale << 3
            )
        # Set the accelerometer resolution
        self.bus.write_byte_data(
            self.address, MPU6050.ACCEL_CONFIG, self.accel_scale << 3
            )
    ...
    def scale_accel(self, raw_acc):
        ...
        return raw_acc / MPU6050.ACCEL_SCALE[self.accel_scale][1]

    def scale_gyro(self, raw_gyro):
        ''' Convert raw gyro value to rad/s '''
        return raw_gyro / MPU6050.GYRO_SCALE[self.gyro_scale][1]
